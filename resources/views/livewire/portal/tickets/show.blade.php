<div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Device Information
                </h3>
                @if ($ticket->device)
                    <p class="text-gray-900 dark:text-white">{{ $ticket->device->brand }} {{ $ticket->device->model }}
                    </p>
                    @if ($ticket->device->serial_number)
                        <p class="text-sm text-gray-600 dark:text-gray-400">Serial: {{ $ticket->device->serial_number }}
                        </p>
                    @endif
                @else
            ...
            <div>
                <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Assigned Technician
                </h3>
            ...
            @if ($ticket->estimated_completion_date)
                <div>
                    <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Estimated
                        Completion</h3>
                    <p class="text-gray-900 dark:text-white">{{ $ticket->estimated_completion_date->format('M d, Y') }}
                    </p>
                </div>
            @endif
        </div>
