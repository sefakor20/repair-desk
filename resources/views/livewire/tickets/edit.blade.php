<div class="max-w-3xl space-y-6">
    ...
    <div>
        ...
        <h1 class="mt-2 text-2xl font-semibold text-zinc-900 dark:text-white">Edit Ticket</h1>
        <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Update ticket information and status</p>
    </div>
    ...
    <form wire:submit="update" class="space-y-6">
        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-800">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea wire:model="form.diagnosis" id="diagnosis" rows="3" placeholder="Technical diagnosis and findings..."
                        class="mt-1.5 block w-full rounded-lg border border-zinc-200 bg-white py-2.5 px-3 text-sm text-zinc-900 placeholder-zinc-400 focus:border-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:placeholder-zinc-500 dark:focus:border-white dark:focus:ring-white"></textarea>
                ...
                <!-- Status -->
                <div>
                    <label for="status" class="block text-sm font-medium text-zinc-900 dark:text-white">
                        Status <span class="text-red-500">*</span>
                    </label>
                    <div class="mt-1.5 grid grid-cols-2 gap-3 sm:grid-cols-5">
                        @foreach ($statuses as $status)
                            <label
                                class="relative flex cursor-pointer rounded-lg border p-4 transition-all focus:outline-none {{ $form['status'] === $status->value ? 'border-zinc-900 bg-zinc-50 dark:border-white dark:bg-zinc-700' : 'border-zinc-200 bg-white hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:bg-zinc-700/50' }}">
                                <input type="radio" wire:model.live="form.status" value="{{ $status->value }}"
                                    class="sr-only" />
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    ...
                    <!-- Actual Completion -->
                    <div>
                        <label for="actual_completion"
                            class="block text-sm font-medium text-zinc-900 dark:text-white">
                            Actual Completion Date (Optional)
                        </label>
                        <input type="date" wire:model="form.actual_completion" id="actual_completion"
                            class="mt-1.5 block w-full rounded-lg border border-zinc-200 bg-white py-2.5 px-3 text-sm text-zinc-900 focus:border-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:focus:border-white dark:focus:ring-white">
                        @error('form.actual_completion')
                            <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit"
                ...
                <svg wire:loading wire:target="update" class="h-4 w-4 animate-spin" fill="none"
