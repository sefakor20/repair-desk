...
test('payment receipt email has correct subject', function (): void {
...
test('payment receipt email uses correct markdown template', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);
...
test('payment receipt email attaches PDF', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);
...
test('payment receipt email includes payment amount', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml('110.00');
});

test('payment receipt email includes transaction reference', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml('TXN-12345');
});

test('payment receipt email includes invoice number', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml($this->invoice->invoice_number);
});

test('payment receipt email includes customer name', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml($this->customer->first_name);
});

test('payment receipt email includes ticket information', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml($this->ticket->ticket_number);
});

test('payment receipt email includes device information', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);
...
test('payment receipt email shows balance due', function (): void {
...
test('payment receipt email shows fully paid status', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);

    $mailable->assertSeeInHtml('Invoice Fully Paid');
});

test('payment receipt email includes portal link when ticket exists', function (): void {
    $mailable = new PaymentReceiptMail($this->payment);
...
test('payment receipt email handles payment without ticket', function (): void {
...
test('payment receipt email is queued', function (): void {
    expect(new PaymentReceiptMail($this->payment))
        ->toBeInstanceOf(\Illuminate\Contracts\Queue\ShouldQueue::class);
});

test('payment receipt email sends successfully', function (): void {
    Mail::fake();
...
test('payment receipt email loads required relationships', function (): void {
