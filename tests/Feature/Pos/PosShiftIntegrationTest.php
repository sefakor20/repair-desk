...
test('sale associates with active shift when shift is open', function () {
...
test('sale does not associate with shift when no shift is open', function () {
    $item = InventoryItem::factory()->create([
        'selling_price' => 100.00,
        'quantity' => 10,
    ]);
...
test('sale does not associate with another users shift', function () {
...
test('shift total sales updates when sale is created', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'total_sales' => 0,
    ]);
...
test('shift sales count increments when sale is created', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'sales_count' => 0,
    ]);
...
test('shift cash sales updates for cash payment', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'cash_sales' => 0,
    ]);
...
test('shift card sales updates for card payment', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'card_sales' => 0,
    ]);
...
test('shift mobile money sales updates for mobile money payment', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
...
test('shift bank transfer sales updates for bank transfer payment', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
...
test('shift totals accumulate across multiple sales', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'total_sales' => 0,
        'sales_count' => 0,
...
test('pos create page shows active shift indicator', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
...
test('pos create page shows no shift warning when no shift is open', function () {
    Livewire::test(Create::class)
        ->assertSee('No Active Shift')
        ->assertSee('Open a shift');
});

test('shift does not update when sale is created without active shift', function () {
...
test('shift totals handle sales with discounts correctly', function () {
    $shift = Shift::factory()->open()->create([
        'opened_by' => $this->user->id,
        'total_sales' => 0,
    ]);
...
test('shift totals handle sales with tax correctly', function () {
